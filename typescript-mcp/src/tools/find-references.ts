...
interface Reference {
  ...
  confidence: number;
  usage_type: 'call' | 'assignment' | 'declaration' | 'parameter' | 'return' | 'property_access' | 'other';
...
export class FindReferencesTool {
  ...
  async call(args: unknown): Promise<FindReferencesResult> {
    try {
      ...
      const indirectRefs = categorizedReferences.filter(r => r.reference_type === 'indirect').length;
      ...
      throw new Error(`Find references failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  private async findAllReferences(entity: any, input: FindReferencesInput): Promise<Reference[]> {
   ...
   * Find references through text search
   */
  private async findTextReferences(entity: any, input: FindReferencesInput): Promise<Reference[]> {
    const references: Reference[] = [];
    
    // Search for entity name in code
    const searchResults = await this.analysisService.searchText(
      entity.codebase_id,
      entity.name,
      {
    ...
    for (const result of searchResults) {
      ...
      if (result.file_path === entity.file_path && 
   ...
   */
  private async findIndirectReferences(entity: any, input: FindReferencesInput): Promise<Reference[]> {
    const references: Reference[] = [];
   ...
   */
  private async findCommentReferences(entity: any, input: FindReferencesInput): Promise<Reference[]> {
    const references: Reference[] = [];
   ...
   */
  private async findStringReferences(entity: any, input: FindReferencesInput): Promise<Reference[]> {
    const references: Reference[] = [];
   ...
   */
  private convertToReference(result: any, type: 'direct' | 'indirect'): Reference {
    return {
  ...
  private async createReferenceFromSearchResult(
    ...
    try {
      ...
      return {
        ...
        containing_function: await this.findContainingFunction(result.file_path, result.line_number),
  ...
  private filterReferences(references: Reference[], input: FindReferencesInput): Reference[] {
    ...
    if (input.exclude_patterns && input.exclude_patterns.length > 0) {
      filtered = filtered.filter(ref => {
        return !input.exclude_patterns!.some(pattern => 
          ref.file_path.includes(pattern)
        );
      });
    }
  ...
  private groupReferencesByFile(references: Reference[]): ReferenceGroup[] {
    ...
    return Array.from(groups.entries()).map(([filePath, refs]) => ({
      file_path: filePath,
  ...
  private findHotspots(groups: ReferenceGroup[]): string[] {
    return groups
      .filter(group => group.total_references > 1)
      .map(group => group.file_path);
  }

  /**
   * Determine usage type from context
   */
  private determineUsageType(context: string): 'call' | 'assignment' | 'declaration' | 'parameter' | 'return' | 'property_access' | 'other' {
    const lowerContext = context.toLowerCase();
    
    if (lowerContext.includes('(') && lowerContext.includes(')')) {
      return 'call';
    }
    if (lowerContext.includes('=') && !lowerContext.includes('==') && !lowerContext.includes('!=')) {
      return 'assignment';
    }
    if (lowerContext.includes('function') || lowerContext.includes('def ') || lowerContext.includes('fn ')) {
      return 'declaration';
    }
   ...
   */
  private async findContainingFunction(filePath: string, lineNumber: number): Promise<string | undefined> {
    try {
   ...
   */
  private async findContainingClass(filePath: string, lineNumber: number): Promise<string | undefined> {
    try {
      const containingEntity = await this.analysisService.findContainingEntity(
        filePath,
        lineNumber,
