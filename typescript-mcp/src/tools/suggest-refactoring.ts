...
import type { RefactoringService } from '../services/refactoring-service.js';
...
const SuggestRefactoringInputSchema = z.object({
  ...
  priority_focus: z.enum(['maintainability', 'performance', 'readability', 'testability']).default('maintainability'),
...
export class SuggestRefactoringTool {
  ...
  private async analyzeCurrentMetrics(entity: any): Promise<CodeMetrics> {
    ...
    return {
      ...
      code_smells: codeSmells.map(smell => smell.description || smell.type || 'Code smell detected')
    };
  }
  ...
  private async generateSuggestions(
    ...
    const refactoringTypes = input.refactoring_types.includes('all') 
      ? ['extract_method', 'extract_class', 'rename', 'move_method', 'inline', 'simplify_conditionals', 'remove_duplicates', 'improve_naming', 'reduce_complexity']
  ...
  private async createExtractMethodSuggestion(
    ...
    const codeExample = input.include_code_examples ? await this.generateExtractMethodExample(entity) : undefined;
    const impactAnalysis = input.include_impact_analysis ? await this.analyzeExtractMethodImpact(entity) : undefined;
  ...
  private async createSimplifyConditionalsSuggestion(
    ...
    return {
      ...
      potential_risks: [
        'Logic changes require careful testing'
      ],
      implementation_steps: [
  ...
  private async createImproveNamingSuggestion(
    ...
    return {
      ...
      potential_risks: [
        'Minimal risk with proper IDE support'
      ],
      implementation_steps: [
  ...
  private async generateExtractMethodExample(entity: any): Promise<CodeExample> {
  ...
  private async analyzeExtractMethodImpact(entity: any): Promise<ImpactAnalysis> {
    return {
  ...
  private sortSuggestionsByPriority(
    ...
    return suggestions.sort((a, b) => {
      ...
      if (focus === 'maintainability') {
        return (b.impact_analysis?.maintainability_improvement || 0) - 
  ...
  private calculateEffortEstimate(suggestions: RefactoringSuggestion[]): EffortEstimate {
    ...
    const complexityLevel = hasLargeEffort ? 'complex' : 
                           totalHours > 16 ? 'moderate' : 'simple';
