...
export class FindDuplicatesTool {
  ...
  private async getFilesToAnalyze(input: FindDuplicatesInput): Promise<string[]> {
    ...
    if (input.file_types && input.file_types.length > 0) {
      files = files.filter(file => 
        input.file_types!.some(ext => file.endsWith(ext))
      );
    }
    ...
    if (input.exclude_patterns && input.exclude_patterns.length > 0) {
      files = files.filter(file => 
        !input.exclude_patterns!.some(pattern => 
  ...
  private async findDuplicateGroups(files: string[], input: FindDuplicatesInput): Promise<DuplicateGroup[]> {
    ...
    for (const detectionType of detectionTypes) {
      const duplicates = await this.duplicationService.findDuplicates(
        files,
        {
  ...
  private generateRefactoringSuggestion(duplicate: any): string {
  ...
  private calculateSavings(duplicate: any): { lines_of_code: number; maintenance_effort: 'low' | 'medium' | 'high' } {
    const instanceCount = duplicate.instances.length;
    const linesPerInstance = duplicate.instances[0].end_line - duplicate.instances[0].start_line + 1;
    const totalDuplicateLines = (instanceCount - 1) * linesPerInstance;
    
    let maintenanceEffort: 'low' | 'medium' | 'high' = 'low';
    if (totalDuplicateLines > 100) maintenanceEffort = 'high';
    else if (totalDuplicateLines > 30) maintenanceEffort = 'medium';
  ...
  private calculateSummary(files: string[], groups: DuplicateGroup[]): DuplicationSummary {
    const filesWithDuplicates = new Set(
      groups.flatMap(g => g.instances.map(i => i.file_path))
    ).size;
    
    const totalDuplicateLines = groups.reduce(
      (sum, g) => sum + g.estimated_savings.lines_of_code, 0
    );
    ...
    const largestDuplicate = Math.max(
      ...groups.map(g => g.estimated_savings.lines_of_code),
      0
    );
  ...
  private generateRecommendations(groups: DuplicateGroup[], summary: DuplicationSummary): string[] {
    ...
    if (summary.duplication_percentage > 15) {
      recommendations.push('High duplication detected. Consider implementing a comprehensive refactoring strategy.');
    }
    
    if (summary.largest_duplicate_size > 50) {
      recommendations.push('Large duplicates found. Prioritize extracting these into reusable components.');
    }
    
    const highPriorityGroups = groups.filter(g => g.estimated_savings.maintenance_effort === 'high');
    if (highPriorityGroups.length > 0) {
      recommendations.push(`${highPriorityGroups.length} high-impact duplicates identified. Address these first.`);
    }
    
    if (summary.potential_code_reduction > 200) {
      recommendations.push(`Potential to reduce codebase by ${summary.potential_code_reduction} lines through deduplication.`);
    }
    
    if (recommendations.length === 0) {
      recommendations.push('Low duplication levels detected. Maintain current code quality practices.');
    }
