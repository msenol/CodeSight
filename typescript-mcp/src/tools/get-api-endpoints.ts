...
import type { ApiDiscoveryService } from '../services/api-discovery-service.js';
...
export class GetApiEndpointsTool {
  ...
  async call(args: unknown): Promise<GetApiEndpointsResult> {
    try {
      ...
      throw new Error(`API endpoint discovery failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
