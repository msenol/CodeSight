...
const CheckComplexityInputSchema = z.object({
  ...
  metric_types: z.array(z.enum(['cyclomatic', 'cognitive', 'lines_of_code', 'maintainability', 'all'])).default(['all']),
...
export class CheckComplexityTool {
  ...
  async call(args: unknown): Promise<ComplexityMetrics> {
    try {
      ...
      const suggestions = input.include_suggestions ? this.generateSuggestions(metrics, rating) : [];
      const comparison = input.include_comparisons ? await this.getBenchmarkComparison(entity, metrics) : undefined;
      ...
      throw new Error(`Complexity check failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private calculateComplexityRating(metrics: any): 'low' | 'medium' | 'high' | 'very_high' {
    let score = 0;
    
    if (metrics.cyclomatic_complexity > 20) score += 3;
    else if (metrics.cyclomatic_complexity > 10) score += 2;
    else if (metrics.cyclomatic_complexity > 5) score += 1;
    
    if (metrics.cognitive_complexity > 25) score += 3;
    else if (metrics.cognitive_complexity > 15) score += 2;
    else if (metrics.cognitive_complexity > 7) score += 1;
    
    if (metrics.lines_of_code > 100) score += 2;
    else if (metrics.lines_of_code > 50) score += 1;
    
    if (metrics.maintainability_index < 20) score += 3;
    else if (metrics.maintainability_index < 40) score += 2;
    else if (metrics.maintainability_index < 60) score += 1;
  ...
  private buildMetricsDetails(metrics: any): MetricDetail[] {
    ...
    if (metrics.cyclomatic_complexity !== undefined) {
      details.push({
        ...
        threshold: 10,
        status: metrics.cyclomatic_complexity > 10 ? 'critical' : metrics.cyclomatic_complexity > 5 ? 'warning' : 'good',
    ...
    if (metrics.cognitive_complexity !== undefined) {
      details.push({
        ...
        threshold: 15,
        status: metrics.cognitive_complexity > 15 ? 'critical' : metrics.cognitive_complexity > 7 ? 'warning' : 'good',
    ...
    if (metrics.lines_of_code !== undefined) {
      details.push({
        ...
        threshold: 50,
        status: metrics.lines_of_code > 100 ? 'critical' : metrics.lines_of_code > 50 ? 'warning' : 'good',
    ...
    if (metrics.maintainability_index !== undefined) {
      details.push({
        ...
        threshold: 60,
        status: metrics.maintainability_index < 40 ? 'critical' : metrics.maintainability_index < 60 ? 'warning' : 'good',
  ...
  private generateSuggestions(metrics: any, rating: string): string[] {
  ...
  private async getBenchmarkComparison(entity: any, metrics: any): Promise<BenchmarkComparison> {
