...
export class DefaultApiDiscoveryService implements ApiDiscoveryService {
  private endpointPatterns: EndpointPattern[] = [
    // Express.js patterns
    {
      framework: 'express',
      patterns: [
        /app\.(get|post|put|delete|patch|options|head)\s*\(\s*['"`]([^'"` ]+)['"`]\s*,\s*([^)]+)\)/g,
        /router\.(get|post|put|delete|patch|options|head)\s*\(\s*['"`]([^'"` ]+)['"`]\s*,\s*([^)]+)\)/g
      ],
    ...
    // Fastify patterns
    {
      framework: 'fastify',
      patterns: [
        /fastify\.(get|post|put|delete|patch|options|head)\s*\(\s*['"`]([^'"` ]+)['"`]\s*,\s*([^)]+)\)/g,
        /server\.(get|post|put|delete|patch|options|head)\s*\(\s*['"`]([^'"` ]+)['"`]\s*,\s*([^)]+)\)/g
      ],
    ...
    // NestJS patterns
    {
      framework: 'nestjs',
      patterns: [
        /@(Get|Post|Put|Delete|Patch|Options|Head)\s*\(\s*['"`]?([^'"`)]*)['"`]?\s*\)/g
      ],
    ...
    // Next.js API routes
    {
      framework: 'nextjs',
      patterns: [
        /export\s+(?:default\s+)?(?:async\s+)?function\s+(\w+)\s*\(\s*req\s*,\s*res\s*\)/g
      ],
  ...
  async detectFrameworks(codebaseId: string): Promise<string[]> {
    ...
    try {
      ...
      for (const filePath of files.slice(0, 10)) { // Sample first 10 files
        try {
          const content = await fs.readFile(filePath, 'utf-8');
          ...
          if (content.includes('export default function') && content.includes('req:') && content.includes('res:')) {
  ...
  private extractExpressEndpoint(match: RegExpMatchArray, code: string, line: number): Partial<APIEndpoint> {
  ...
  private extractFastifyEndpoint(match: RegExpMatchArray, code: string, line: number): Partial<APIEndpoint> {
    const method = match[1].toUpperCase();
    const path = match[2];
    const handlerCode = match[3];
  ...
  private extractNestJSEndpoint(match: RegExpMatchArray, code: string, line: number): Partial<APIEndpoint> {
    const method = match[1].toUpperCase();
    const path = match[2] || '';
    
    // Find the method name after the decorator
    const afterDecorator = code.substring(match.index! + match[0].length);
    const methodMatch = afterDecorator.match(/\s*(\w+)\s*\(/); 
  ...
  private extractNextJSEndpoint(match: RegExpMatchArray, code: string, line: number): Partial<APIEndpoint> {
  ...
  private extractKoaEndpoint(match: RegExpMatchArray, code: string, line: number): Partial<APIEndpoint> {
    const method = match[1].toUpperCase();
    const path = match[2];
    const handlerCode = match[3];
    ...
    try {
      ...
      this.traverseAST(ast, (node: any) => {
        ...
        if (node.type === 'CallExpression' && node.callee) {
          const endpoint = this.extractEndpointFromCallExpression(node, filePath);
          if (endpoint) {
        ...
        if (node.type === 'Decorator') {
          const endpoint = this.extractEndpointFromDecorator(node, filePath);
          if (endpoint) {
            endpoints.push(endpoint);
          }
        }
  ...
  private extractResponses(handlerCode: string): string[] {
    ...
    const statusCodes = handlerCode.match(/res\.status\s*\(\s*(\d+)\s*\)/g) || [];
  ...
  private extractNextJSMethods(code: string): string[] {
    ...
    const methodChecks = code.match(/req\.method\s*===\s*['"`](\w+)['"`]/g) || [];
  ...
  private isInGraphQLContext(content: string, index: number): boolean {
    ...
    return beforeIndex.includes('resolvers') || 
  ...
  private async extractEndpointsFromFile(filePath: string, content: string): Promise<APIEndpoint[]> {
    const endpoints: APIEndpoint[] = [];
  ...
  private traverseASTForEndpoints(node: any, endpoints: APIEndpoint[], filePath: string, content: string): void {
    if (!node || typeof node !== 'object') return;
    ...
    if (node.type === 'CallExpression' &&
        ...
        ['get', 'post', 'put', 'delete', 'patch', 'head', 'options'].includes(node.callee.property.name)) {
    ...
    if (node.type === 'Decorator' &&
        ...
        ['Get', 'Post', 'Put', 'Delete', 'Patch', 'Head', 'Options'].includes(node.expression.callee.name)) {
  ...
  private extractEndpointsWithRegex(content: string, endpoints: APIEndpoint[], filePath: string): void {
    const lines = content.split('\n');
    
    // Express patterns
    const expressPattern = /\b(app|router)\.(get|post|put|delete|patch|head|options)\s*\(\s*['"\`]([^'"\`]+)['"\`]/gi;
    
    // NestJS patterns
    const nestjsPattern = /@(Get|Post|Put|Delete|Patch|Head|Options)\s*\(\s*['"\`]?([^'"\`\)]*)['"\`]?\s*\)/gi;
