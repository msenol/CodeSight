...
export interface RefactoringService {
  ...
  suggestExtractMethod(
    filePath: string,
    startLine: number,
    endLine: number,
  ): Promise<RefactoringSuggestion[]>;
  suggestExtractVariable(
    filePath: string,
    line: number,
    expression: string,
  ): Promise<RefactoringSuggestion[]>;
...
export class DefaultRefactoringService implements RefactoringService {
    ...
    try {
      ...
      for (const pattern of this.refactoringPatterns) {
        ...
        for (const match of matches) {
          suggestions.push({
            id: this.generateId(),
            type: pattern.name.toLowerCase().replace(/]+/g, '_'),
            priority: 'medium',
            file: filePath,
            line: match.line,
  ...
  async suggestExtractMethod(
    ...
    const returnType =
      returnValues.length > 1 ? `{${returnValues.join(', ')}}` : returnValues[0] || 'void';
...
${returnValues.length > 0 ? `    return ${returnValues.length > 1 ? `{${returnValues.join(', ')}}` : returnValues[0]};` : ''}
      ...
      for (const pattern of this.codeSmellPatterns) {
        ...
        for (const match of matches) {
          suggestions.push({
            ...
            confidence:
              pattern.severity === 'high' ? 0.9 : pattern.severity === 'medium' ? 0.7 : 0.5,
          });
        }
      }
    } catch (error) {
  ...
  private detectUnnecessaryVariables(code: string, ast?: any): RefactoringMatch[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const varMatch = line.match(/^]*(const|let|var)]+(\w+)]*=]*(.+);?$/);
        ...
        if (
          ...
          if (node.params.length > 5) {
            ...
            matches.push({
              ...
              suggestion:
                'Consider using parameter objects or breaking the function into smaller functions',
            });
          }
        }
      });
  ...
  private detectDeadCode(code: string, ast?: any): CodeSmellMatch[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const varDeclaration = line.match(/^]*(const|let|var)]+(\w+)/);
      if (varDeclaration) {
      ...
      for (const variable of declaredVariables) {
        if (
          ...
          !line.match(new RegExp(`^]s*(const|let|var)]s+${variable}`))
        ) {
      ...
      this.traverseAST(ast, (node: any) => {
        if (node.type === 'ClassDeclaration' && node.body) {
          const methods = node.body.body.filter(
            (member: any) => member.type === 'MethodDefinition',
          );
  ...
  private extractVariables(code: string): string[] {
    ...
    const varPattern = /\b(const|let|var)]+(\w+)/g;
    let match;
  ...
  private detectFactoryPatternOpportunity(code: string): boolean {
    ...
    const newStatements = (code.match(/new]+\w+]*\(/g) || []).length;
    const conditionals = (code.match(/if]*\(/g) || []).length;
  ...
  private async analyzeWithTextPatterns(
    content: string,
    filePath: string,
  ): Promise<RefactoringSuggestion[]> {
    const suggestions: RefactoringSuggestion[] = [];
  ...
  private analyzeWithRegex(
    ...
    const functionPattern = /function]+(\w+)|const]+(\w+)]*=]*\(/g;
    let match;
  ...
  private detectLongMethodSmells(
    ast: any,
    content: string,
    smells: CodeSmell[],
    filePath: string,
  ...
  private detectLargeClassSmells(
    ast: any,
    content: string,
    smells: CodeSmell[],
    filePath: string,
  ...
  private detectDeepNestingSmells(
    ast: any,
    content: string,
    smells: CodeSmell[],
    filePath: string,
