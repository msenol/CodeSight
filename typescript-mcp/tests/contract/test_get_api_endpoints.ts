...
describe('MCP Tool: get_api_endpoints - Contract Tests', () => {
  ...
  describe('Response Schema Validation', () => {
    ...
    it('should return APIEndpoint objects conforming to schema', async () => {
      ...
      if (body.endpoints.length > 0) {
        ...
        const validMethods = ['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'OPTIONS', 'HEAD', 'GraphQL'];
        ...
        expect(endpoint.file_path).toMatch(/\.(ts|js|py|java|cpp|c|rs|go|cs|php|rb|swift|kt|scala|dart|ex)$/);
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle GraphQL endpoints', async () => {
      ...
      const graphqlEndpoints = body.endpoints.filter((endpoint: APIEndpoint) => endpoint.method === 'GraphQL');
    ...
    it('should handle mixed REST and GraphQL APIs', async () => {
      ...
      if (body.endpoints.length > 0) {
        ...
        const hasRest = ['GET', 'POST', 'PUT', 'DELETE', 'PATCH'].some(method => methods.has(method));
